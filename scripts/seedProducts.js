...
const PRODUCT_SETS = [
  ...
  {
    name: 'Dantaparta',
    description: 'Elegant coconut shell toothbrush holder for your bathroom, keeping your toothbrushes organized and your counter tidy.',
    items: '1 toothbrush holder',
    countInStock: 0,
    manufactureCost: 150,
    packagingCost: 35,
    profitMultiplier: 2,
    ...
    gstPercentage: 5,
    category: 'lifestyle',
    image: '/assets/kosha/BATHROOM/toothbrush-holder/1.png',
    images: [
      '/assets/kosha/BATHROOM/toothbrush-holder/1.png',
    ]
  }
...
function getProductSets() {
  return PRODUCT_SETS.map((set, index) => ({
    ...
    // Use provided rate after discount for retail price
    retailPrice: Math.round(set.retailPrice),
    mrp: Math.round(set.mrp), // MRP for strikethrough display
...
async function seedProducts() {
  ...
  // Remove all legacy kosha products first
  await Product.deleteMany({ category: 'kosha' });
  // Also remove existing products in new categories to avoid duplicates during re-seed
  await Product.deleteMany({ category: { $in: ['bowls', 'candles', 'glassware', 'cutlery', 'lifestyle'] } });

  // Products to feature as New Arrivals
  const newArrivalNames = ['Santidīpa', 'Madhuri', 'Varnikapatra', 'Mahapatra'];

  // Add user field and log pricing details
  const productsWithUser = products.map(p => {
    // console.log(`${p.name}:`);
    // console.log(`  MRP: ₹${p.mrp}`);
    // console.log(`  Retail Price: ₹${p.retailPrice}`);
    // console.log(`  Stock: ${p.countInStock}`);
    // console.log(`  Items: ${p.items}`);
    // console.log('');

    const isNewArrival = newArrivalNames.includes(p.name);
    // If it's a new arrival, give it a fresh timestamp (now).
    // If not, give it an older timestamp (7 days ago) to ensure it appears later in the sort.
    const createdAt = isNewArrival ? new Date() : new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);

    return {
      ...p,
      user: admin._id,
      createdAt,
      updatedAt: createdAt // Keep consistent
    };
  });

  await Product.insertMany(productsWithUser);

  // DUPLICATE specific products into 'kosha' category so it's not empty
  const koshaProducts = products.filter(p => ['Kalāpatra', 'Santidīpa', 'Snehadātrī', 'Jala Patram Duo Set'].includes(p.name)).map(p => ({
    ...p,
    category: 'kosha',
    _id: undefined // Let Mongo generate new IDs
  }));

  const koshaProductsWithUser = koshaProducts.map(p => {
    return {
      ...p,
      user: admin._id,
  ...
  await Product.insertMany(koshaProductsWithUser);

  console.log(`Seeded ${productsWithUser.length} main products and ${koshaProductsWithUser.length} Kosha duplicates.`);
  process.exit();
}
